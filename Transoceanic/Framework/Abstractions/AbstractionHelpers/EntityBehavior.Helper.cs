    ...
    where TBehavior : SingleEntityBehavior<TEntity>
{
    ...
    public bool TryGetBehavior(TEntity entity, string methodName, [NotNullWhen(true)] out TBehavior behavior) =>
        (behavior = entity is not null && _data.TryGetValue(entity.EntityType, out SimpleEntityBehaviorSet<TEntity, TBehavior> set) ? set.GetFirstBehavior(entity, methodName) : null) is not null;
