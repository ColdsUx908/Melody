...
public static class TONPCUtils
{
    ...
    public static bool IsEnemy(this NPC npc) => npc.active && !npc.friendly && npc.lifeMax >= 5;
    ...
    /// <returns></returns>
    public static bool IsBossTO(this NPC npc) => npc.active && (npc.boss || npc.IsEoW() || npc.IsWallOfFleshEye());

    public static bool IsEoW(this NPC npc) => npc.active && npc.type is NPCID.EaterofWorldsHead or NPCID.EaterofWorldsBody or NPCID.EaterofWorldsTail;

    public static bool IsWallOfFleshEye(this NPC npc) => npc.active && npc.type == NPCID.WallofFleshEye;

    public static bool IsRetinazer(this NPC npc) => npc.active && npc.type == NPCID.Retinazer;

    public static bool IsSpazmatism(this NPC npc) => npc.active && npc.type == NPCID.Spazmatism;

    public static bool IsDefeatingEoW(NPC npc) => npc.type switch
    {
        NPCID.EaterofWorldsHead => TOEntityIteratorCreator.NewNPCIterator(k => k.IsEoW(), npc).Count() <= 1,
        NPCID.EaterofWorldsBody => TOEntityIteratorCreator.NewNPCIterator(k => k.IsEoW(), npc).Count() <= 2,
        NPCID.EaterofWorldsTail => TOEntityIteratorCreator.NewNPCIterator(k => k.IsEoW(), npc).Count() <= 1,
        _ => false
    };
