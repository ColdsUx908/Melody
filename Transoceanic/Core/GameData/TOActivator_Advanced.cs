...
public static partial class TOActivator
{
    ...
    /// <param name="radian">单次旋转角度（逆时针）。</param>
        ...
        IEntitySource source, Vector2 position, Vector2 velocity, int type, int damage, float knockback, int owner = -1)
    {
        ...
        for (int i = 0; i < number; i++)
        {
            if (NewProjectileActionCheck(out int index, out Projectile projectile, source, position, velocity.RotatedBy(radian * i), type, damage, knockback, owner))
            {
                indexes[i] = index;
    ...
    /// <param name="radian">单次旋转角度（逆时针）。</param>
    /// <returns>Projectile是否全部生成。</returns>
    public static bool RotatedProjCheck(out List<int> indexes, out List<Projectile> projectiles, out int spawnedNumber, int number, float radian,
        IEntitySource source, Vector2 position, Vector2 velocity, int type, int damage, float knockback, int owner = -1)
    {
        indexes = [];
        projectiles = [];
