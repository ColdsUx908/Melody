...
public class TOLoaderSystem : ModSystem
{
    public override void PostAddRecipes()
    {
        foreach (IResourceLoader loader in
            ...
            orderby pair.type.GetMethod("PostAddRecipes", TOReflectionUtils.UniversalBindingFlags)?.GetAttribute<LoadPriorityAttribute>()?.Priority ?? 0
    ...
    public override void OnWorldLoad()
    {
        foreach (IResourceLoader loader in
            from pair in TOReflectionUtils.GetTypesAndInstancesDerivedFrom<IResourceLoader>().AsValueEnumerable()
            orderby pair.type.GetMethod("OnWorldLoad", TOReflectionUtils.UniversalBindingFlags)?.GetAttribute<LoadPriorityAttribute>()?.Priority ?? 0 descending
            select pair.instance)
        {
            loader.OnWorldLoad();
        }
