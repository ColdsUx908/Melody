...
public sealed class ScalRitualDrama_Tweak : CAProjectileTweak<SCalRitualDrama>
{
    ...
    public override bool PreAI()
    {
        ...
        {
            for (int i = 0; i < 2; i++)
                ParticleHandler.SpawnParticle(new BloomParticle1(Projectile.Center, Vector2.Zero, Color.Lerp(Color.Red, Color.Magenta, 0.3f), 0f, 0.55f, 270, false));
        ...
        if (Projectile.timeLeft == 689 - 180)
            ParticleHandler.SpawnParticle(new BloomParticle1(Projectile.Center, Vector2.Zero, new Color(121, 21, 77), 0f, 0.85f, 90, false));
